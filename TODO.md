...
- [ ] Спроектировать модель хранения нескольких homes на `UUID` игрока.
- [ ] Добавить лимит по умолчанию и лимиты через permissions.
- [ ] Реализовать `/sethome <name>` и сохранить совместимость с `/sethome` без аргументов.
- [ ] Реализовать `/home <name>` как вывод координат, без телепортации.
- [ ] Реализовать `/homes` со списком, количеством `used/limit` и миром.
- [ ] Реализовать `/delhome <name>`.
- [ ] Добавить tab-complete по именам домов игрока.
- [ ] Добавить сообщения об ошибках: лимит превышен, дом не найден, имя занято.

### Acceptance Criteria

- [ ] Старое поведение `/sethome` и `/home` не ломается.
- [ ] Игрок не может создать больше домов, чем разрешено его правами.
- [ ] `/homes` показывает корректный список только домов текущего игрока.
- [ ] Удаление дома обновляет хранилище и tab-complete.
...
- [ ] Добавить секцию `info-card` в config.
- [ ] Реализовать сбор biome/time/weather для сохраненной локации.
- [ ] Добавить creator/created-at для homes и warps, если данных ещё нет.
- [ ] Сформировать hover-card или расширенный chat-output.
- [ ] Учитывать выключенные поля из конфига.
- [ ] Не выполнять тяжёлые операции на main thread, если появятся внешние custom-fields.

### Acceptance Criteria

- [ ] `/home` и `/warp <name>` показывают расширенную информацию при включенном `info-card`.
- [ ] Отключенные поля не попадают в сообщение.
- [ ] Отсутствующие metadata не вызывают ошибки.
